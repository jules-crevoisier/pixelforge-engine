...
const OBJECTIFS = [
  ...
  },
  {
    ...
    criteres: [
      ...
      { nom: 'Le bureau : Linux, Windows, macOS par l’échafaudage Electron',
        symboles: ['paquetBureau', 'pageDeJeu', "'paquet:bureau'"],
        indices: ['paquet de bureau porte la page', 'fenetre nait a la taille',
          'zip electron', 'meme avec'],
        preuves: 3 },
      { nom: 'Le code s’édite dehors : scripts en fichiers, relus à chaque Jouer',
        symboles: ['scriptsVersFichiers', 'appliquerFichiersScripts', 'retenirDepart', 'ecrireSous'],
        indices: ['script devient un fichier', 'verbatim', 'idempotent', 'modifie dehors',
          'note, pas jete', 'les scripts en vrais fichiers', 'version editee dehors',
          'survit a l’arret'],
        preuves: 6 },
      ...
      { nom: 'L’épée, les cœurs, la réapparition et les pointes se débrayent',
        ...
        indices: ['sans epee', 'pointes deviennent du decor', 'quatre regles traversent',
          'defauts sont ceux'],
        preuves: 4 },
      { nom: 'Les fichiers se voient, s’ouvrent et se déposent, comme dans un moteur',
        symboles: ['PanneauFichiers', 'routerFichier', 'lireFichier', 'genreDe'],
        indices: ['panneau Fichiers inventorie', 'atelier de dessin DESSUS',
          'deposer une image sur la page', 'deposer un .json'],
        preuves: 4 },
      { nom: 'Les niveaux de Tiled et de LDtk entrent, jouables',
        symboles: ['ajouterCarteImporteeProjet', 'depuisTiled', 'depuisLdtk', 'sceneAppariee'],
        indices: ['niveau tiled devient une carte', 'scene appariee arrive avec le heros',
          'meme fichier ne l', 'projet ldtk depose', 'tiled depose'],
        preuves: 4 },
      { nom: 'L’arbre de scène : retrouver, renommer, cacher, réordonner, retirer',
        symboles: ['blocScene', 'reglerNoeudProjet', 'retirerNoeudProjet', 'decalerNoeudProjet'],
        indices: ['noeud se renomme', 'noeud se cache', 'ordre des freres', 'racine, elle, ne se retire pas',
          'renommer un noeud', 'la retirer depuis', 'noeud se duplique', 'centre la vue',
          'la surligne'],
        preuves: 7 },
      { nom: 'Les assemblages : un prefab qui se pose depuis la palette',
        symboles: ['poserAssemblageProjet', 'retirerAssemblageProjet', 'remplirAssemblages',
          'AssemblageSerialise'],
        indices: ['assemblage nomme', 'modele est une copie', 'garde ses assemblages',
          'palette propose le modele', 'identifiants neufs, jamais deux pareils',
          'reprend la copie'],
        preuves: 5 },
      { nom: 'L’audio enregistré entre aussi : un .wav devient un son du projet',
        symboles: ['dechiffrerWav', 'base64DepuisOctets', 'ajouterSonImporteProjet', 'wav?: string'],
        indices: ['dechiffreur parlent le meme WAV', 'son importe se REND', 'wav illisible retombe',
          'wav traverse', 'deposer un .wav'],
        preuves: 4 },
      { nom: 'Un seul journal : le Ctrl+Z traverse une reconstruction du projet',
        symboles: ['adresseCases', 'adresseSolides', 'Resolveur', 'gesteStructure',
          'adopterId', 'POIDS_GARDE'],
        indices: ['tableau D\'AUJOURD\'HUI', 'adresse morte n', 'noeud relu garde',
          'identifiant deja pris', 'defait le redimensionnement',
          'ctrl+z suivant retrouve le coup de pinceau', 'posee avant la reconstruction',
          'salle tiree de travers', 'memoire de l', 'trois derniers gestes'],
        preuves: 8 },
      { nom: 'L’inspecteur : ce qu’un nœud porte se lit et se règle',
        symboles: ['blocInspecteur', 'proprietes:', 'editerScript', 'surChoixNoeud',
          'dessinerSelection'],
        indices: ['inspecteur deplace un noeud', 'proprietes du sac survivent',
          'retirer explicitement', 'pose et retire un script', 'ouvre l\'inspecteur SUR',
          'champs communs ET les proprietes', 'regler x deplace', 'sans que l\'inspecteur la connaisse',
          'atelier sur ce noeud-la'],
        preuves: 7 },
      { nom: 'Le clavier agit sur le nœud choisi : flèches, Suppr, Ctrl+D, F',
        symboles: ['deplacerChoisi', 'retirerChoisi', 'dupliquerChoisi', 'fusionDeplacement',
          'get dernier'],
        indices: ['fleche deplace le noeud choisi', 'maj+fleche', 'pressions fusionnent',
          'ctrl+d duplique', 'copie qui est choisie', 'suppr retire le noeud',
          'remet choisi', 'f centre la vue', 'echap le deselectionne',
          'fleches deplacent la vue'],
        preuves: 8 },
      { nom: 'La console dit ce qui ne va pas, et le garde',
        symboles: ['PanneauConsole', 'signaler', 'surTrace', 'fautesScripts', 'MESSAGES_GARDES'],
        indices: ['console s', 'exception levee pendant que le jeu tourne',
          'identique se compte', 'c.tracer', 'refus dit par quoi le remplacer',
          'laisse une trace', 'vider', 'interdits : il ne passerait pas',
          'arrive mis en forme'],
        preuves: 7 },
      { nom: 'L’arbre COMPOSE : créer un nœud, changer son parent en le glissant',
        symboles: ['ajouterNoeudProjet', 'reparenterNoeudProjet', 'TYPES_NOEUD', 'draggable'],
        indices: ['cinq types de noeud', 'cree un noeud nu', 'sous un autre',
          'valeurs par defaut du moteur', 'propre parent', 'enfant de ce qu',
          'racine ne se reparente pas', 'cree un noeud de groupe',
          'donne pour parent', 'emmene son corps', 'ressort du groupe'],
        preuves: 8 },
      { nom: 'Renommer SUIT les références — et dit ce qu’il n’a pas su suivre',
        symboles: ['renommerEspeceProjet', 'renommerPlancheProjet', 'scriptsQuiNomment'],
        indices: ['porte le nom de son espece', 'entites deja posees suivent',
          'nom deja pris est refuse', 'especes qui y piochent',
          'emmene ses salles', 'declencheurs', 'sprite dont la planche porte le meme nom',
          'signale, pas reecrit', 'portent le nouveau nom'],
        preuves: 7 },
      { nom: 'Figer une image, et l’avancer d’un pas',
        symboles: ['enPauseMaintenant', 'unPas', 'reprendre()', 'majPause'],
        indices: ['fige la partie', 'avance d\'un pas', 'trois clics font trois pas',
          'image qui arrive apres la pause', 'pause tourne toujours',
          'collisions se voient par-dessus', 'sans bond de rattrapage',
          'hors pause, le pas a la main'],
        preuves: 6 },
      { nom: 'Le brouillon : un onglet fermé ne coûte pas l’heure qu’on vient de passer',
        symboles: ['poserBrouillon', 'listerBrouillons', 'oublierBrouillons',
          'BROUILLONS_GARDES', 'deposerBrouillon'],
        indices: ['depose un brouillon', 'base locale du navigateur',
          'projet inchange ne fait pas', 'plus vieux part', 'pas un enregistrement',
          'propose le brouillon', 'reprendre ouvre le projet perdu', 'tout oublier'],
        preuves: 7 },
      { nom: 'Copier-coller des nœuds, d’une scène à l’autre',
        symboles: ['collerNoeudProjet', 'copieAIdentifiantsNeufs', 'pressePapiers', 'collerIci'],
        indices: ['coller ajoute le sous-arbre', 'identifiants que personne',
          'coller deux fois', 'autre scene la peuple', 'scene inconnue ne fabrique rien',
          'ctrl+v colle le noeud', 'presse-papiers traverse', 'reprend le collage'],
        preuves: 7 },
      { nom: 'Trouver : un seul champ pour tout ce que le projet nomme',
        symboles: ['chercherDansProjet', 'TROUVAILLES_MAX', 'normaliser', 'Trouvaille'],
        indices: ['question vide', 'carte se trouve par son nom', 'noeud de scene se trouve',
          'position rendue est absolue', 'centre a viser', 'nom exact passe devant',
          'ignore les accents', 'meme ordre', 'liste est bornee',
          'ctrl+f ouvre', 'entree fait le trajet', 'change de carte et centre'],
        preuves: 9 },
      { nom: 'Plusieurs nœuds à la fois : rectangle, Ctrl+clic, et un seul geste',
        symboles: ['surSelectionRect', 'noeudsChoisis', 'basculerChoix', 'rectangleChoisit'],
        indices: ['choisit tout ce que le rectangle couvre', 'vue les entoure toutes',
          'fleche deplace tous', 'seul ctrl+z les rend toutes', 'retire tous les noeuds choisis',
          'les remet tous', 'ctrl+clic en ajoute', 'ctrl+clic sur le meme'],
        preuves: 7 },
      { nom: 'Les salles se tirent à la souris : déplacer, retailler, défaire',
        symboles: ['poigneeSalle', 'tirerSalle', 'glisseSalle', 'tireeDepart'],
        indices: ['quatre bords se distinguent', 'quatre coins aussi',
          'trois cases garde un interieur', 'bord ouest deplace l',
          'jamais sous deux cases', 'interieur la deplace', 'bord la retaille',
          'seul ctrl+z defait toute la retaille'],
        preuves: 6 },
      { nom: 'Le dessin de sprite entre au journal — le détail ne se perd plus',
        symboles: ['poserGeste', 'finirTrait', 'caseVivante', 'avantTrait'],
        indices: ['ajoute du detail au pinceau', 'defait le trait',
          'refaire le remet', 'defont le trait puis', 'structure fait apres le dessin',
          'case ajoute une case', 'ctrl+z la reprend'],
        preuves: 6 },
      { nom: 'Voir ce qu’on s’apprête à poser, et qui l’on règle',
        symboles: ['dessinerFantome', 'globalAlpha', 'rectDeTuile'],
        indices: ['apercu de ce qu', 'saisir une entite la choisit'],
        preuves: 2 },
  ...
  },
  {
    ...
    criteres: [
      ...
      { nom: 'Des déclencheurs : les événements du niveau en données',
        symboles: ['Declencheurs', 'ajouterDeclencheurProjet', 'declencheursDeSalle', 'instantane'],
        indices: ['declencheur', 'declencheurs', 'franchissement', 'rembobiner', 'une fois'],
        preuves: 10 },
      { nom: 'Un script peut tout ce qu’un jeu fait',
        symboles: ['jouer(son: string', 'dire(dialogue: string', 'poserEntite', 'ouvrirDialogue'],
        indices: ['c.jouer', 'verbes', 'tire en jouant'], preuves: 2 },
      { nom: 'Plusieurs cartes, un déroulé : un projet devient un jeu',
        symboles: ['ajouterCarteProjet', 'renommerCarteProjet', 'allerCarte', 'prochaineCarte',
          'carteSuivante', 'interfaceOuverte'],
        indices: ['carte neuve', 'toutes les cartes', 'deroule', 'ecran-titre', 'niveau deux',
          'sous le pinceau'],
        preuves: 12 },
      { nom: 'Une lumière fidèle à la palette, au prix mesuré',
        symboles: ['TableLumiere', 'Eclairage', 'lumiereEn', 'lueur'],
        indices: ['lumiere', 'nuit', 'palette', 'ambiante', 'plein jour', 'assombri'],
        preuves: 12 },
      { nom: 'Un jeu-témoin complet, joué par les bancs',
        symboles: ['projetGouffre', 'carteDepuisPlan', 'PLANS'],
        indices: ['jeu-temoin', 'clairiere', 'se traversent', 'pieds sur du sol', 'generateur', 'artefact', 'du jeu'],
        preuves: 8 },
      { nom: 'Un projet relu meurt et REVIENT, comme un vrai jeu',
        symboles: ['Aventure', 'reapparition', 'ephemere', 'consommesLocaux'],
        indices: ['reprise', 'revient', 'ephemere', 'meme touche reste servie'],
        preuves: 6 },
      { nom: 'Salles et lumière par carte : la carte partout',
        symboles: ['reglerAmbianteCarteProjet', 'sallesPour', 'ambiante: number | null'],
        indices: ['memes cases sur deux cartes', 'ambiante', 'epaissit'],
        preuves: 8 },
      ...
      { nom: 'Les textes et les musiques s’écrivent dans l’éditeur',
        symboles: ['ajouterDialogueProjet', 'ajouterMusiqueProjet', 'ecouterMusique', 'blocDialogues'],
        indices: ['dialogue neuf', 'musique neuve', 'au panneau', 'les notes se reglent'],
        preuves: 6 },
      { nom: 'Le premier lancement se comprend sans rien apprendre',
        symboles: ['accueilPlateforme', 'AIDE_OUTILS', 'dessinerCadreEdition', 'caseSurvolee'],
        indices: ['accueil, trois departs', 'nait jouable', 'vient de peindre', 'pied de page',
          'sous le curseur'],
        preuves: 4 },
  ...
  },
  {
    ...
    criteres: [
      ...
      { nom: 'Un jeu à plusieurs niveaux s’exporte entier',
        symboles: ['charger_carte', 'ChargerCarte', 'niveau_suivant', 'NiveauSuivant'],
        indices: ['trois niveaux et leurs scenes', 'changer de carte', 'premiere carte du deroule'],
        preuves: 4 },
